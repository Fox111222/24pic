...
cc.Class({
    ...
    showHarm: function(harmStr) {
        ...
        flyWord.showHarm(harmStr, cc.v2(0, 0), this.node.scaleX);
        */
    },
    ...
    setModelID: function(num) {
        /*
        this.modelID = num;
        if(this.modelID == 0) {   //modelID 是0表示服务器传来的人物pipi_player；他的scalex=1;且初始向左
            this.leftDir = 1;     //所以给scalex=this.leftDir则维持向左
            this.rightDir = -1;
        }else if(this.modelID == 1) {//modelID 是1表示服务器传来的人物pop_player；他的scalex=-1;且初始向左
            this.leftDir = -1;       //所以给scalex=this.leftDir则转为向右
            this.rightDir = 1;
        }
        else if(this.modelID == 2) {
            this.leftDir = -1;       //他的scalex=-1;且初始向左
            this.rightDir = 1;
        }else if(this.modelID == 3) {//他的scalex=-1;且初始向右
            this.leftDir = -1;
            this.rightDir = 1;
        }
        */
    ...
    onStopWalk: function(pos) {
        ...
        if(pos.x != this.node.x) {
            ...
            var action = cc.sequence(move,finished);
    ...
    getItemPoint: function() {
        ...
        this.itemPoint = this.node.parent.convertToNodeSpaceAR(this.itemPoint);
    ...
    onMousePickUpItem: function(item, itemID, pickPos) {
        ...
        cc.log("AvatarAction.onMousePickUpItem")
    ...
    adjustArrowDir: function(pos, center) {
        /*
        cc.log("AvatarAction.adjustArrowDir curpos----centerps=",pos.x,pos.y,center.x,center.y)
        ...
        if(this.node.scaleX == this.rightDir) {
            this.arrow.scaleX = this.rightDir;
            factor = (this.modelID==0 || this.modelID==2) ? 1 : -1;
        } else if(this.node.scaleX == this.leftDir) {
            this.arrow.scaleX = this.leftDir;
        ...
        cc.log("this.arrowAngle",this.arrowAngle)
        ...
        //this.camera.getComponent("CameraControl").increase()
        */
    },
    ...
    throw: function(pos) {
        ...
        var impulse = null;
        /////////////////////////
        this.startThrowPoint=this.getArrowWorldPoit();
        ///////////////////////////////
    ...
    throwItem: function(item, impulse) {
        ...
        //this.camera.getComponent("cameraControl").decrease()
        */
    },

    touchPickItem: function(touchPos) {
        /*
        cc.log("touchPickItem")
        var minDistance = 0.0;
        var item = null;
        for(var i = 0; i < this.items.length; i++) {
            //var distance = cc.pDistance(this.items[i].position, this.node.position);
            var start=cc.v2(this.items[i].position.x, this.items[i].position.y)
            var end=cc.v2(this.node.position.x, this.node.position.y)
        ...
        if(item) {
            var polyCollider=item.getComponent(cc.PolygonCollider);
            var isHited = cc.Intersection.pointInPolygon(touchPos, polyCollider.world.points);
            //var ret = cc.sys.isMobile ? true : (event.getButton() === cc.Event.EventMouse.BUTTON_LEFT);
            //cc.log("ret=/this.canPicked=/isHited=/this.player",ret , this.canPicked ,isHited , this.player)
            if(!isHited) {
                return
            }
    ...
    touchAdjustThrow: function(touchPos) {
        /*
        cc.log("AvatarAction.touchAdjustThrow")
       
        if(this.startThrowPoint) {
            cc.log("this.startThrowPoint endpos=",this.startThrowPoint.x,this.startThrowPoint.y,touchPos.x,touchPos.y)
            //this.adjustArrowDir(touchPos, this.startThrowPoint);
            this.adjustArrowDir(touchPos, this.getArrowWorldPoit());
    ...
    update: function(dt) {
        ...
        if(this.arrow.active) {
            ...
            this.item.angle=-this.arrowAngle*this.node.scaleX;
        }
        ...
        for (var i = 0; i < results.length; i++) {
            ...
            if(collider.node.name == "land_bg") {
                ...
                if(!ret) {
                    ...
                    var foot_point = this.node.parent.convertToNodeSpaceAR(result.point);
