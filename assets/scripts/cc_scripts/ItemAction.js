...
cc.Class({
    ...
    onLoad () {
       ...
       this.draw .parent=this.node
        ...
        if(cc.sys.isMobile) {
            cc.log("cc.sys.isMobile TOUCH_START, this.pickUped")
            this.node.on(cc.Node.EventType.TOUCH_START, this.pickUped, this);
        } else {
            cc.log("cc.sys.isMobile cc.Node.EventType.MOUSE_DOWN, this.pickUped")
 ...
 //           this.draw.getComponent(cc.Graphics).drawPoly(this.phyPolyCollider.points, cc.color(100, 0, 0, 50), 1, cc.color(0, 0, 0, 125));
            this.canPicked = true;
 ...
 //           this.draw.getComponent(cc.Graphics).drawPoly(this.phyPolyCollider.points, cc.color(100, 0, 0, 50), 1, cc.color(0, 0, 0, 125));
    ...
    onCollisionExit: function (other, self) {
        if(self.tag == 110 && this.player && other.node.name === this.player.name && this.playerControl &&this.playerControl.isEnable()) {
            //this.draw.clear();
            this.draw.getComponent(cc.Graphics).clear();
            this.canPicked = false;
    ...
    onBeginContact: function (contact, selfCollider, otherCollider) {
        if( (otherCollider.node.name == PIPI_NAME || otherCollider.node.name == POP_NAME || otherCollider.node.name == "archerPrefab" || otherCollider.node.name == "2ArcherPrefab") && this.isThrowed) { //扣血
    ...
    onPreSolve: function (contact, selfCollider, otherCollider) {
        if( (otherCollider.node.name == PIPI_NAME || otherCollider.node.name == POP_NAME || otherCollider.node.name == "archerPrefab" || otherCollider.node.name == "2ArcherPrefab") && this.isThrowed) {
    ...
    pickUped: function(event) {
        cc.log("ItemAction.pickUped")
        //var pickPos = this.camera.getCameraToWorldPoint( event.getLocation());
        var pickPos=cc.Vec2(0,0)
        this.camera.getScreenToWorldPoint(event.getLocation(),pickPos)
